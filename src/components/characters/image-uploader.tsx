...
import { uploadImage } from '@/app/characters/actions';
...
export default function ImageUploader({
  ...
  return (
    <div
      className={`relative aspect-[3/4] w-full max-w-sm rounded-lg border-2 border-dashed p-4 text-center transition-colors ${isDragging ? 'border-primary bg-primary/10' : 'border-border hover:border-primary'}`}
      onPaste={onPaste}
    ...
    >
      {imageUrl ? (
        <>
          <Image src={imageUrl} alt="Character portrait" fill className="rounded-md object-cover" />
          ...
          <div className="flex flex-col items-center justify-center">
            {isUploading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">You can also paste an image from clipboard</p>
              </>
            )}
          </div>
